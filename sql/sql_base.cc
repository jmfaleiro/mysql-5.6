          ...
          const char *key, uint key_length, TABLE_SHARE **share)
{
  ...
  *share= NULL;
  Table_cache *tc= table_cache_manager.get_cache(thd);
  tc->lock();
  TABLE *table= tc->get_table(thd, hash_value, key, key_length, share);

  if (!table && *share)
  {
      /*
       * Using the same logic in open_table(), which acquires LOCK_open
...
bool get_table_from_cache(THD *thd, TABLE_LIST *table_list)
{
  ...
  my_hash_value_type hash_value= my_calc_hash(&table_def_cache, (uchar*) key, key_length);
