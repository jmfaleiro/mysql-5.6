...
void Log_event::do_add_to_dag(Relay_log_info *rli, Log_event_wrapper *ev)
{
  ...
  if (ev->get_begin_event())
  {
    ...
    rli->prev_event->next_group_event= ev;
  }
  else
  {
...
{
  Log_event_wrapper *ev_iter= NULL;
...
//  DBUG_ASSERT(!rli->dag.is_empty() || ev->is_begin_event);
  ...
  for (ev_iter= rli->current_begin_event; ev_iter->next_group_event != NULL;
       ev_iter= ev_iter->next_group_event)
  {
  }
  DBUG_ASSERT(ev_iter == ev);
...
void Log_event::do_post_end_event(Relay_log_info *rli, Log_event_wrapper *ev)
{
  ...
  /* If this is a sync group, we're going to execute a global barrier.
   * Wait for the dag to drain. */
  ...
  // Log_event_wrapper *begin_event= ev->get_begin_event();
  rli->begin_event_list.push_back(rli->current_begin_event);
  ...
  // signal a change in DAG
  mysql_mutex_lock(&rli->dag_group_ready_mutex);
  rli->dag_group_ready= true;
  mysql_cond_signal(&rli->dag_group_ready_cond);
  mysql_mutex_unlock(&rli->dag_group_ready_mutex);

  /* We just scheduled a sync group. Wait for it to execute. */
  if (rli->dag_sync_group)
  {
    mysql_mutex_lock(&rli->dag_empty_mutex);
    while (!rli->dag_empty)
...
/**
  Use attribute to silence unused function warnings. We might need this
  function to assert things in to the future.
  */
static
my_bool are_any_columns_signaled_for_key(KEY *keyinfo, MY_BITMAP *cols)
__attribute__((unused));

/**
  Returns true if any primary columns are signaled in the provided bitmap.
 */
static
my_bool are_any_columns_signaled_for_key(KEY *keyinfo, MY_BITMAP *cols)
{
  DBUG_ENTER("are_any_columns_signaled_for_key");
...
*/
bool
Rows_log_event::get_keys(Relay_log_info *rli, std::deque<Dependency_key> &keys)
{
  ...
  if (replication_type_options == REPLICATION_TYPE_TABLE)
  {
    keys.push_back(table_key);
    DBUG_RETURN(TRUE);
  }

  DBUG_ASSERT(replication_type_options == REPLICATION_TYPE_STATEMENT);
  ...
  if (!get_table_ref(rli, &memory, &table_list))
  {
    ...
    got_keys= do_get_keys(rli, table_list, keys);
                                   ...
                                   RPL_TABLE_LIST **table_list)
{
  ...
  Table_map_log_event *prev= rli->last_table_map_event;
  ...
  *memory= prev->setup_table_rli(table_list);
  ...
  bool success = get_table_from_cache(thd, (TABLE_LIST*)(*table_list));
...
void Rows_log_event::do_add_to_dag(Relay_log_info *rli, Log_event_wrapper *ev)
{
  ...
  for (auto k : m_keylist)
  {
    auto it= rli->dag_key_last_penultimate_event.find(k);
    if (it == rli->dag_key_last_penultimate_event.end())
    {
      continue;
    }

    auto last_penultimate_event= it->second; 
    last_penultimate_event->add_child(ev);
  }
  rli->dag_unlock();
...
#if !defined(MYSQL_CLIENT) && defined(HAVE_REPLICATION)
bool
Write_rows_log_event::do_get_keys(Relay_log_info *rli,
    RPL_TABLE_LIST *table_list, std::deque<Dependency_key> &keys)
{
  MY_BITMAP *cols= NULL;
  TABLE *table= NULL;
  uint primary_key;
  uchar *curr_row, *key_buf;
  ...
  DBUG_ENTER("Write_rows_log_event::do_get_keys");
...
#if !defined(MYSQL_CLIENT) && defined(HAVE_REPLICATION)
bool Delete_rows_log_event::do_get_keys(Relay_log_info *rli,
          RPL_TABLE_LIST *table_list, std::deque<Dependency_key> &keys)
{
  MY_BITMAP *cols= NULL;
  TABLE *table= NULL;
  ...
  DBUG_ENTER("Delete_rows_log_event::do_get_keys");
...
#if !defined(MYSQL_CLIENT) && defined(HAVE_REPLICATION)
bool Update_rows_log_event::do_get_keys(Relay_log_info* rli,
        RPL_TABLE_LIST *table_list, std::deque<Dependency_key>& keys)
{
  MY_BITMAP *cols= NULL;
  TABLE *table= NULL;
  ...
  DBUG_ENTER("Update_rows_log_event::do_get_keys");
  ...
  keyinfo= &table->key_info[primary_key];
  //DBUG_ASSERT(!are_any_columns_signaled_for_key(keyinfo, &m_cols_ai));
  got_keys= true;
  curr_row= m_rows_buf;
